...
class RunTimeNode(Node):
    ...
    def get_publishers(self, node):
        info = get_publisher_info(
                    node=node, remote_node_name=self.name)
...
class ParserNodes(rclpy.node.Node):
    ...
    def timer_callback(self):
        ...
        for n in node_names:
            ...
            self.get_logger().info(f"name: {new_n.name}, {new_n.namespace}, {new_n.full_name}")
            parsed_node = RunTimeNode(node=Node(name=new_n.name,
                                                namespace=new_n.namespace,
                                                full_name=new_n.full_name))
            ...
            f_path = Path(str(folder) + f'{new_n.full_name}.json')
